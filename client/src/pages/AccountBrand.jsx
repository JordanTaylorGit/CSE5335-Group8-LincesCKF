  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="font-display text-xl text-navy">Brand Dashboard</h2>
        <Link to="add-item" className="px-4 py-2 bg-navy text-white rounded-md text-sm">Add Item</Link>
      </div>

      <p className="text-sm text-navy/70 mb-4">Signed in as: <strong>{user?.companyName || user?.name}</strong></p>
      ...
      {loading ? (
        ...
        <div className="p-6 border border-dashed border-zinc-200 rounded-md">
          <p className="text-navy/70">You have not added any products yet.</p>
          <Link to="add-item" className="inline-block mt-4 px-4 py-2 bg-silk-amber text-navy rounded-md">Create your first product</Link>
        </div>
      ) : (
